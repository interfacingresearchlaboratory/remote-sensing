...
C2 = L7.merge(L5) \
.filterDate('2005-01-01', '2007-01-01') \
.sort('DATE_ACQUIRED') \
.filter(ee.Filter.lt('CLOUD_COVER', 10)) \
.select(['B5', 'B4']) \
.mean() \
...
C3 = L8 \
.filterDate('2008-01-01', '2020-10-10') \
.sort('DATE_ACQUIRED') \
.filter(ee.Filter.lt('CLOUD_COVER',10)) \
.select(['B6', 'B5']) \
.mean() \
...
#Composite
NDBIchange = ee.Image.cat(C1, C2, C3)
NDBIchange2 = NDBIchange.multiply(1)

vizParams = {
  'min': -0.25,
  'max': 0.25
}
Map.addLayer(NDBIchange2, vizParams, 'NDBIchange')
Map